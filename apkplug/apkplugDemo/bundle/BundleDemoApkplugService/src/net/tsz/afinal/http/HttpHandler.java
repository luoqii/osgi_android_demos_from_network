...
public class  HttpHandler  <T> extends  AsyncTask<Object, Object, Object> implements EntityCallBack{
	...
	private void handleResponse(HttpResponse response) {
		...
		if (status.getStatusCode() >= 300) {
			...
			//onProgressUpdate(UPDATE_FAILURE,new HttpResponseException(status.getStatusCode(), status.getReasonPhrase()),status.getStatusCode() ,errorMsg);
			...
			try {
				...
				//onProgressUpdate(UPDATE_SUCCESS,responseBody);
			} catch (IOException e) {
				//onProgressUpdate(UPDATE_FAILURE,e,0,e.getMessage());
